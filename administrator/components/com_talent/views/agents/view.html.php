...
class TalentViewAgents extends JViewLegacy {
	protected $canDo;
	function display($tpl = null) {
		// Get application
		$app = JFactory::getApplication ();
		$context = "talent.list.admin.agents";
		...
		// Set the submenu
		TalentHelper::addSubmenu ( 'agents' );
	...
	protected function addToolBar() {
		$title = JText::_ ( 'COM_TALENT_MANAGER_AGENTS' );
		if ($this->pagination->total) {
			$title .= "<span style='font-size: 0.5em; vertical-align: middle;'>(" . $this->pagination->total . ")</span>";
		}
		JToolBarHelper::title ( $title, 'agent' );
		...
		if ($this->canDo->get ( 'core.delete' )) {
			JToolBarHelper::deleteList ( 'Do you really want to delete?', 'agents.delete', 'JTOOLBAR_DELETE' );
		}
